...
public class RecurrencePattern : EncodableDataType
{
    ...
    public RecurrencePattern(FrequencyType frequency, int interval = 1) : this()
    {
        Frequency = frequency;
        Interval = interval;
    }
    ...
    {
        base.CopyFrom(obj);
        if (obj is not RecurrencePattern pattern)
        {
            return;
        }
        ...
        ByMillisecond = new List<int>(pattern.ByMillisecond);
