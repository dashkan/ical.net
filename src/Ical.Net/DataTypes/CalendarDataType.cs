...
public abstract class CalendarDataType : ICalendarDataType
{
    ...
    {
        if (!(obj is ICalendarDataType calendarDataType))
        {
            return;
        }

        _AssociatedObject = calendarDataType.AssociatedObject;
        _proxy.SetParent(_AssociatedObject);
        _proxy.SetProxiedObject(calendarDataType.Parameters);
    }
