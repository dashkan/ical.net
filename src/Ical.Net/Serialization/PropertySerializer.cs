...
public class PropertySerializer : SerializerBase
{
    ...
    public override string SerializeToString(object obj)
    {
        ...
        foreach (var v in prop.Values.Where(value => value != null))
        {
            ...
            var valueSerializer = (IStringSerializer)sf.Build(v.GetType(), SerializationContext);

            // Iterate through each value to be serialized,
            // and give it a property (with parameters).
            // TODO: this isn't always the way this is accomplished.
            ...
            // TODO: the "parameter modification" operation should
            ...
            if (v is ICalendarDataType calendarDataType)
            {
            ...
            if (v is PeriodList periodList)
            {
                if (!string.IsNullOrWhiteSpace(periodList.TzId) && parameterList.All(p => string.Equals("TZID", p.Value, StringComparison.OrdinalIgnoreCase)))
                {
