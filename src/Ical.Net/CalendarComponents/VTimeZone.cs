...
public sealed class VTimeZone : CalendarComponent
{
    ...
    public static VTimeZone FromDateTimeZone(string tzId, DateTime earliestDateTimeToSupport, bool includeHistoricalData)
    {
        ...
        else
        {
            ...
            var latestStandardInterval = standardIntervals.MaxBy(x => x.Start);
            ...
            if (latestStandardInterval != null && (latestStandardInterval.HasEnd ? latestStandardInterval.End : Instant.MaxValue) != Instant.MaxValue)
            {
                ...
                {
                    var latestDaylightInterval = daylightIntervals.MaxBy(x => x.Start);
        ...
        bool isOnlyInterval = false)
    {
        ...
        var oldestInterval = matchedIntervals.MinBy(x => x.Start);
    ...
    public Uri Url
    {
        get => _url ??= Properties.Get<Uri>("TZURL");
        set
        {
            _url = value;
